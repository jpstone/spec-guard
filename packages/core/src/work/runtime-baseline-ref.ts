...
// Greenfield (new_entirely) establishes its runtime baseline as a BOOTSTRAP DELIVERABLE after
// authorization (the dev runtime can only be proven once the scaffold exists), so while none exists yet
// the pre-approval / pre-authorization "accepted baseline required" gate is exempt. modify_existing
// inherits the already-proven singleton and still requires it. (RUNTIME_BASELINE_ORIGINATION_DESIGN.md.)
export function greenfieldBaselineDeferred(work: WorkPacket): boolean {
  return work.runtime_baseline_ref === null && work.origination === "new_entirely";
}
